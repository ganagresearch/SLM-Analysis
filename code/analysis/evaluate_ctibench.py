...
# Potentially reuse or adapt refusal checking from evaluate_sevenllm.py if needed
# REFUSAL_PATTERNS = [...]
# NON_REFUSAL_PATTERNS = [...]
# REFUSAL_REGEX = re.compile(...)
# NON_REFUSAL_REGEX = re.compile(...)

# def is_response_llm_refusal(response: str) -> bool:
#     """Checks if the response is likely a refusal."""
#     # Implementation adapted from other scripts or simplified
#     pass # Placeholder
...
def evaluate_cti_task(
    ...
    if task_type == 'mcq':
        ...
        # Case sensitivity might matter for CVSS vectors, use exact match
    ...
    for i, item in enumerate(model_outputs):
        ...
        if item_id is None:
            logging.warning(f"Skipping output item at index {i}: missing 'item_id'.")
            continue
        ...
        if predicted_answer is None:
            ...
            logging.warning(f"Could not parse answer for item_id: {item_id}.")
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate CTIBench benchmark results.")
    # Changed arguments to directories
    parser.add_argument("--input_dir", required=True, type=Path, help="Directory containing model _outputs.json and _metrics.json files.")
    parser.add_argument("--ground_truth_dir", required=True, type=Path, help="Directory containing ground truth JSONL files (e.g., cti-mcq_ground_truth.jsonl).")
    ...
    # --- Find Output Files ---
    glob_pattern = str(args.input_dir / f"*{args.benchmark_filter}*_outputs.json")
    logging.info(f"Searching for output files in: {args.input_dir} using pattern: {glob_pattern}")
    output_files = glob.glob(glob_pattern)

    if not output_files:
        logging.warning(f"No '*_outputs.json' files matching the filter '{args.benchmark_filter}' found in {args.input_dir}. Exiting.")
        exit(0)

    logging.info(f"Found {len(output_files)} potential output files to process.")

    # --- Ensure output directory exists ---
    # Create a subdirectory based on the input directory name for better organization
    eval_output_subdir = args.output_dir / args.input_dir.name
    ...
    for output_file_path_str in output_files:
        ...
        match = re.match(r'ctibench_cti-(mcq|rcm|vsp|ate)', output_file_path.name, re.IGNORECASE)
        if not match:
        ...
        metrics_file_path = args.input_dir / f"{base_name}_metrics.json"
        gt_file_name = f"cti-{task_type}_ground_truth.jsonl" # Assumes GT filenames match task type
        ground_truth_file_path = args.ground_truth_dir / gt_file_name
        ...
        if metrics_data and isinstance(metrics_data, dict):
            ...
            if model_path_in_metrics:
                model_name = parse_model_name(model_path_in_metrics)
            ...
            ts_match = re.search(r'(\d{8}-\d{6})', output_file_path.name)
            timestamp_str = ts_match.group(1) if ts_match else timestamp_str
            # Maybe try to infer model name from path if possible? For now, UnknownModel.

        logging.info(f"Model: {model_name}, Timestamp: {timestamp_str}, Task: {task_type}")
        ...
        try:
            ...
            final_summary = {
                 ...
                 **run_args, # Include run arguments if available
    ...
    if all_run_summaries:
        combined_summary_path = eval_output_subdir / f"_COMBINED_summary_{args.input_dir.name}.json"
